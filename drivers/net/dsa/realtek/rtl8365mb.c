...
/* VLAN 4k table entry */
#define RTL8367C_VLAN_4K_ENTRY_SIZE			3 /* 48-bits */
#define RTL8367C_VLAN_4K_CONF0_MEMBERS_LS_MASK		GENMASK(7, 0)
#define RTL8367C_VLAN_4K_CONF2_MEMBERS_MS_MASK		GENMASK(2, 0)
#define RTL8367C_VLAN_4K_CONF0_UNTAG_LS_MASK		GENMASK(15, 8)
#define RTL8367C_VLAN_4K_CONF2_UNTAG_MS_MASK		GENMASK(5, 3)
#define RTL8367C_VLAN_4K_CONF1_FID_MSI_MASK		GENMASK(3, 0)
#define RTL8367C_VLAN_4K_CONF1_VBPEN_MASK		GENMASK(4, 4)
#define RTL8367C_VLAN_4K_CONF1_VBPRI_MASK		GENMASK(7, 5)
#define RTL8367C_VLAN_4K_CONF1_ENVLANPOL_MASK		GENMASK(8, 8)
#define RTL8367C_VLAN_4K_CONF1_METER_IDX_LS_MASK	GENMASK(13, 9)
#define RTL8367C_VLAN_4K_CONF2_METER_IDX_MS_MASK	GENMASK(6, 6)

/* VLAN MC registers */
#define RTL8367C_VLAN_MC_CONF_BASE			0x0728
#define RTL8367C_VLAN_MC_CONF_ENTRY_SIZE 		4 /* 64-bit */
#define RTL8367C_VLAN_MC_CONF_REG(index) \
		(RTL8367C_VLAN_MC_CONF_BASE + \
		 RTL8367C_VLAN_MC_CONF_ENTRY_SIZE * (index))
#define RTL8367C_VLAN_MC_CONF_SIZE 			32
#define  RTL8367C_VLAN_MC_CONF0_MEMBERS_MSK		GENMASK(10, 0)
#define  RTL8367C_VLAN_MC_CONF1_FID_MSI_MSK		GENMASK(3, 0)
#define  RTL8367C_VLAN_MC_CONF2_VBPEN_MSK		GENMASK(0, 0)
#define  RTL8367C_VLAN_MC_CONF2_VBPRI_MSK		GENMASK(3, 1)
#define  RTL8367C_VLAN_MC_CONF2_ENVLANPOL_MSK		GENMASK(4, 4)
#define  RTL8367C_VLAN_MC_CONF2_METER_IDX_MSK		GENMASK(10, 5)
#define  RTL8367C_VLAN_MC_CONF3_EVID_MSK		GENMASK(12, 0)

enum rtl8365mb_table {
	RTL8367C_TABLE_ACL_RULE = 1,
	RTL8367C_TABLE_ACL_ACT,
	RTL8367C_TABLE_CVLAN, /* 4k vlan table */
	RTL8367C_TABLE_L2,
	RTL8367C_TABLE_IGMP_GROUP,
...
static const int rtl8365mb_table_entry_size[] = {
	[RTL8367C_TABLE_ACL_RULE] = 0,
	[RTL8367C_TABLE_ACL_ACT] = 0,
	[RTL8367C_TABLE_CVLAN] = 3,
	[RTL8367C_TABLE_L2] = 0,
	[RTL8367C_TABLE_IGMP_GROUP] = 0
};
...
{
	/* vlan4k.vid = vlan->vid; */
	vlan4k->member = FIELD_GET(RTL8367C_VLAN_4K_CONF0_MEMBERS_LS_MASK, buf[0]) |
		  (FIELD_GET(RTL8367C_VLAN_4K_CONF2_MEMBERS_MS_MASK, buf[2]) <<
		   FIELD_WIDTH(RTL8367C_VLAN_4K_CONF0_MEMBERS_LS_MASK));
	vlan4k->untag = FIELD_GET(RTL8367C_VLAN_4K_CONF0_UNTAG_LS_MASK, buf[0]) |
		   (FIELD_GET(RTL8367C_VLAN_4K_CONF2_UNTAG_MS_MASK, buf[2]) <<
		    FIELD_WIDTH(RTL8367C_VLAN_4K_CONF0_UNTAG_LS_MASK));

	vlan4k->fid = FIELD_GET(RTL8367C_VLAN_4K_CONF1_FID_MSI_MASK, buf[1]);
	/* vlan4k->vlan_based_pri_enabled = FIELD_GET(RTL8367C_VLAN_4K_CONF1_VBPEN_MASK, buf[1]); */
	/* vlan4k->priority = FIELD_GET(RTL8367C_VLAN_4K_CONF1_VBPRI_MASK, buf[1]); */
	/* vlan4k->vlan_policy_enabled = FIELD_GET(RTL8367C_VLAN_4K_CONF1_ENVLANPOL_MASK, buf[1]); */
	/* vlan4k->meter_idx = FIELD_GET(RTL8367C_VLAN_4K_CONF1_METER_IDX_LS_MASK, buf[1]) |
		    (FIELD_GET(RTL8367C_VLAN_4K_CONF2_METER_IDX_MS_MASK, buf[2]) <<
		     FIELD_WIDTH(RTL8367C_VLAN_4K_CONF1_METER_IDX_LS_MASK));
	*/
}

static void rtl8365mb_vlan4k_buf(struct rtl8366_vlan_4k *vlan4k, u16 *buf)
{
	buf[0] &= ~RTL8367C_VLAN_4K_CONF0_MEMBERS_LS_MASK;
	buf[0] |= FIELD_PREP(RTL8367C_VLAN_4K_CONF0_MEMBERS_LS_MASK,
				    vlan4k->member & FIELD_MAX(RTL8367C_VLAN_4K_CONF0_MEMBERS_LS_MASK));
	buf[2] &= ~RTL8367C_VLAN_4K_CONF2_MEMBERS_MS_MASK;
	buf[2] |= FIELD_PREP(RTL8367C_VLAN_4K_CONF2_MEMBERS_MS_MASK,
				    vlan4k->member >> FIELD_WIDTH(RTL8367C_VLAN_4K_CONF0_MEMBERS_LS_MASK));

	buf[1] &= ~RTL8367C_VLAN_4K_CONF1_FID_MSI_MASK;
	buf[1] |= FIELD_PREP(RTL8367C_VLAN_4K_CONF1_FID_MSI_MASK,
				    vlan4k->fid);

	/*buf[1] &= ~RTL8367C_VLAN_4K_CONF1_VBPRI_MASK;
	buf[1] |= FIELD_PREP(RTL8367C_VLAN_4K_CONF1_VBPRI_MASK,
				    vlan4k->priority);*/

	buf[0] &= ~RTL8367C_VLAN_4K_CONF0_UNTAG_LS_MASK;
	buf[0] |= FIELD_PREP(RTL8367C_VLAN_4K_CONF0_UNTAG_LS_MASK,
				    vlan4k->untag & FIELD_MAX(RTL8367C_VLAN_4K_CONF0_UNTAG_LS_MASK));
	buf[2] &= ~RTL8367C_VLAN_4K_CONF2_UNTAG_MS_MASK;
	buf[2] |= FIELD_PREP(RTL8367C_VLAN_4K_CONF2_UNTAG_MS_MASK,
				    vlan4k->untag >> FIELD_WIDTH(RTL8367C_VLAN_4K_CONF0_UNTAG_LS_MASK));
}
...
{
	u16 vlan_entry[RTL8367C_VLAN_4K_ENTRY_SIZE] = {0, 0, 0};
	struct realtek_priv *priv = ds->priv;
	...
	if (vlan->vid > RTL8365MB_MAX_4K_VID) {
		if (extack)
			NL_SET_ERR_MSG_MOD(extack, \
				   "VLAN ID greater than " \
				    STR(RTL8365MB_MAX_4K_VID));
		return -EINVAL;
	}

	ret = rtl8365mb_table_access(priv, RTL8367C_TABLE_CVLAN,
...
{
	vlanmc->member = FIELD_GET(RTL8367C_VLAN_MC_CONF0_MEMBERS_MSK, buf[0]);
	/* vlan_mc does not have untag info in this device */
	/* vlanmc->untag = ?? */
	vlanmc->fid = FIELD_GET(RTL8367C_VLAN_MC_CONF1_FID_MSI_MSK, buf[1]);
	vlanmc->priority = FIELD_GET(RTL8367C_VLAN_MC_CONF2_VBPRI_MSK, buf[2]);
	vlanmc->vid = FIELD_GET(RTL8367C_VLAN_MC_CONF3_EVID_MSK, buf[3]);
}

static void rtl8365mb_vlanmc_buf(struct rtl8366_vlan_mc *vlanmc, u16 *buf)
{
	buf[0] &= ~RTL8367C_VLAN_MC_CONF0_MEMBERS_MSK;
	buf[0] |= FIELD_PREP(RTL8367C_VLAN_MC_CONF0_MEMBERS_MSK, vlanmc->member);

	buf[1] &= ~RTL8367C_VLAN_MC_CONF1_FID_MSI_MSK;
	buf[1] |= FIELD_PREP(RTL8367C_VLAN_MC_CONF1_FID_MSI_MSK, vlanmc->fid);

	buf[2] &= ~RTL8367C_VLAN_MC_CONF2_VBPRI_MSK;
	buf[2] |= FIELD_PREP(RTL8367C_VLAN_MC_CONF2_VBPRI_MSK, vlanmc->priority);

	buf[3] &= ~RTL8367C_VLAN_MC_CONF3_EVID_MSK;
	buf[3] |= FIELD_PREP(RTL8367C_VLAN_MC_CONF3_EVID_MSK, vlanmc->vid);
}
...
static int rtl8365mb_vlanmc_set(struct dsa_switch *ds, int port,
			      ...
			      struct netlink_ext_ack *extack, bool include)
{
	u16 vlan_entry[RTL8367C_VLAN_MC_CONF_ENTRY_SIZE] = {0};
	...
	if (vlan->vid > RTL8365MB_MAX_MC_VID) {
		if (extack)
			NL_SET_ERR_MSG_MOD(extack, "VLAN ID greater than "
					STR(RTL8365MB_MAX_MC_VID));

		return -EINVAL;
	}

	/* look for existing entry or an empty one */
	for (vlanmc_idx = 0; vlanmc_idx < RTL8367C_VLAN_MC_CONF_SIZE; vlanmc_idx++) {
		ret = regmap_bulk_read(priv->map,
				       RTL8367C_VLAN_MC_CONF_REG(vlanmc_idx),
				       &vlan_entry,
				       RTL8367C_VLAN_MC_CONF_ENTRY_SIZE);
		if (ret) {
			if (extack)
				NL_SET_ERR_MSG_MOD(extack,
		...
		evid = FIELD_GET(RTL8367C_VLAN_MC_CONF3_EVID_MSK, vlan_entry[3]);
	...
	if (vlanmc_idx == RTL8367C_VLAN_MC_CONF_SIZE) {
		...
		if (first_unused < 0) {
			if (extack)
				NL_SET_ERR_MSG_MOD(extack,
					   "All VLAN MC entries (" \
					   STR(RTL8367C_VLAN_MC_CONF_SIZE) \
					   ") are in use.");
			return -EINVAL;
		}
	...
	dev_dbg(priv->dev, "Current port PVID @%d acpt frame %d\n",
